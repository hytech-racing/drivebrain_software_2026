...
struct DriverInput
{
    float requested_accel;  // float from 0 to 1 representing percent of accel
                            // pedal travel
    float requested_brake;  // float from 0 to 1 representing pedal travel of
                            // brake
};
...
class StateTracker
{
   public:
    ...
    /**
     * Receives a protobuf message and adds any useful information to the
     * internal vehicle state.
     *
     * @param message the message with information to add.
     */
    void handle_receive_protobuf_message(
   ...
   private:
    ...
    void _handle_set_inverter_dynamics(
        std::shared_ptr<google::protobuf::Message> msg);

    template <size_t index, typename inverter_temps_message>
    void _handle_set_inverter_temps(
        std::shared_ptr<google::protobuf::Message> msg);

    template <size_t arr_len>
    bool _validate_timestamps(
        const std::array<std::chrono::microseconds, arr_len>& timestamp_arr);

    void _receive_low_level_state(
        std::shared_ptr<google::protobuf::Message> message);

    void _receive_inverter_states(
        std::shared_ptr<google::protobuf::Message> message);
